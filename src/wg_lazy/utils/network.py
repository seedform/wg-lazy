...
class PrivateIPv4Address(IPv4Network):
    ...
    _ERROR_MSG_PREFIX = "invalid private IPv4 address"
    ...
    def __init__(
        ...
        try:
            super().__init__(cidr_notation_addr, strict=False)
        except ValueError:
...
class AssignableIPv4Address(IPv4Address):
    ...
    Address cannot be multicast, reserved, or from an unspecified
    category.
    """
    ...
    def __init__(self, addr: str):
        ...
        invalid = any(
            [self.is_multicast, self.is_reserved, self.is_unspecified]
        )
